...
class MdCorAtualizadorSeiRN extends InfraRN
{
    ...
    private $historicoVersoes = array('1.0.0', '1.1.0', '2.0.0');
    ...
    private function instalarv100()
    {
        ...
        foreach ($arrDados as $status => $dados) {
            foreach ($dados as $tipo => $dado) {
                foreach ($dado as $desc => $nomeImagem) {
                    ...
                    $objMdCorListaStatusDTO->setNumIdMdCorListaStatus($numMdCorListaStatus++);
                    $objMdCorListaStatusDTO->setNumStatus($status);
                    $objMdCorListaStatusDTO->setStrTipo($tipo);
                    $objMdCorListaStatusDTO->setStrDescricao($desc);
                    $objMdCorListaStatusDTO->setStrNomeImagem($nomeImagem);
                    $objMdCorListaStatusDTO->setStrSinAtivo('S');
                    $objMdCorListaStatusRN->cadastrar($objMdCorListaStatusDTO);
                }
            }
        }
        ...
        $arrTabelas = array('md_cor_adm_par_ar_infrigen', 'md_cor_adm_parametro_ar', 'md_cor_ar_cobranca', 'md_cor_contato',
            'md_cor_contrato', 'md_cor_diretoria', 'md_cor_embalagem', 'md_cor_expedicao_andamento', 'md_cor_expedicao_anexo',
            'md_cor_expedicao_formato', 'md_cor_expedicao_solicitad', 'md_cor_extensao_midia', 'md_cor_justificativa',
            'md_cor_lista_status', 'md_cor_map_unid_servico', 'md_cor_map_unidade_exp', 'md_cor_objeto', 'md_cor_parametro_rastreio',
            'md_cor_plp', 'md_cor_retorno_ar', 'md_cor_retorno_ar_doc', 'md_cor_serie_exp', 'md_cor_servico_postal', 'md_cor_status_process',
            'md_cor_substatus_process', 'md_cor_tipo_correspondenc', 'md_cor_tipo_embalagem', 'md_cor_tipo_objeto', 'md_cor_unidade_exp');
...
try {
    ...
    if (!isset($arrConfig['SEI']['Modulos'])) {
        ...
        if (!key_exists('CorreiosIntegracao', $arrModulos)) {
            throw new InfraException('MÓDULO DO CORREIOS NÃO DECLARADO NA CONFIGURAÇÃO DO SEI');
        }
    }

    if (!class_exists('CorreiosIntegracao')) {
        throw new InfraException('A CLASSE PRINCIPAL "CORREIOSINTEGRACAO" DO MÓDULO DO CORREIOS NÃO ENCONTRADA');
    }

    $objVersaoRN = new MdCorAtualizadorSeiRN();
