...
//  $strItensSelUnidade =  InfraINT::montarSelectArrInfraDTO($strPrimeiroItemValor, $strPrimeiroItemDescricao, $strValorItemSelecionado, $arrObjMdCorMapeamentoUniExpSolDTO, 'IdUnidadeExp', 'SiglaUnidadeSolicitante');
    ...
    if ($numRegistros > 0) {
        ...
        $strResultado .= '<th class="infraTh" width="44%">Unidade Solicitante</th>' . "\n";
        $strResultado .= '<th class="infraTh" width="40%">Serviços Postais</th>' . "\n";
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            if ($arrObjMdCorMapUnidServicoDTO[$i]->getStrSinAtivo() == 'N') {
            ...
            if ($bolCheck) {
                $strResultado .= '<td valign="top">' . PaginaSEI::getInstance()->getTrCheck($i, $arrObjMdCorMapUnidServicoDTO[$i]->getNumIdUnidadeSolicitante(), $arrObjMdCorMapUnidServicoDTO[$i]->getNumIdUnidadeSolicitante()) . '</td>';
            }
            $strResultado .= '<td>';
            ...
            if ($bolAcaoDesativar && $arrObjMdCorMapUnidServicoDTO[$i]->getStrSinAtivo() == 'S') {
            ...
            if ($bolAcaoReativar && $arrObjMdCorMapUnidServicoDTO[$i]->getStrSinAtivo() == 'N') {
...
?>
    <form id="frmMdCorMapUnidServicoLista" method="post"
            ...
            <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                <div class="row">
                    <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
                        ...
                        <select class="infraSelect form-control" tabindex="504" name="selUnidade"
                                onchange="enviarFormPesquisa()"
                                id="selUnidade"<?= $strDesabilitarConsultar ?>>
                            <?= $strItensSelUnidade ?>
                        </select>
                    </div>
                    <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
                        <label class="infraLabelOpcional">&nbsp;Serviço Postal:</label>
                        <select class="infraSelect form-control" tabindex="504" name="selServico"
                                onchange="enviarFormPesquisa()"
                                id="selServico"<?= $strDesabilitarConsultar ?>>
                            <?= $strItensSelServico ?>
                        </select>
                    </div>
                </div>
