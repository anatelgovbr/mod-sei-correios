<script>
    ...
    function inicializar() {
        ...
        formato += '        <div id="divRdoGravacao" class="infraRadioDiv">';
        ...
        impressao += '              </div>';
        impressao += '              <label id="lblImpressao_' + id + '" for="rdoImpressao1_' + id + '" class="infraLabelRadio" tabindex="507">Preto e Branco</label>';
        impressao += '          </div>';
        impressao += '    </div>';
        impressao += '    <div class="col-sm-12 col-md-12 col-lg-7 col-xl-7">';
        impressao += '          <div id="divRdoGravacao" class="infraRadioDiv ">';
        ...
        impressao += '          </div>';
        impressao += '          <label id="lblImpressao_' + id + '" for="rdoImpressao2_' + id + '" class="infraLabelRadio" tabindex="507">Colorido</label>';
        impressao += '    </div>';
        impressao += '</div>';

        //HTML para a coluna justificativa
        var justificativa = '<textarea name="txtJustificativa[' + id + ']" class="infraText" style="width: 200px;" disabled="disabled"></textarea>';
        ...
        formato += '        <div id="divRdoImpressao" class="infraDivRadio">';
        formato += '            <div class="infraRadioDiv ">';
        formato += '                <input id="rdoImpresso_' + id + '" <?php if ($formatoDTO->getStrFormaExpedicao() == MdCorExpedicaoFormatoRN::$TP_FORMATO_IMPRESSO) echo 'checked'; ?> '+ checkImpressao +'  value="<?php echo MdCorExpedicaoFormatoRN::$TP_FORMATO_IMPRESSO; ?>" type="radio" name="rdoFormato_' + id + '" onclick="impressaoMostrar()" ' + visualizarListagemPLP +'>';
        formato += '                <label class="infraRadioLabel" for="rdoImpresso_' + id + '"></label>';
        formato += '            </div>';
        formato += '            <label id="lblImpresso_' + id + '" for="rdoImpresso_' + id + '" class="infraLabelRadio" tabindex="507">Impresso</label>';
        formato += '        </div>';
        ...
        formato += '    <div class="col-sm-12 col-md-12 col-lg-7 col-xl-7">';
        formato += '        <div id="divRdoGravacao" class="infraRadioDiv">';
        formato += '            <input id="rdoMidia_' + id + '" <?php if ($formatoDTO->getStrFormaExpedicao() == MdCorExpedicaoFormatoRN::$TP_FORMATO_MIDIA) echo 'checked'; ?>  value="<?php echo MdCorExpedicaoFormatoRN::$TP_FORMATO_MIDIA; ?>" type="radio" name="rdoFormato_' + id + '" onclick="impressaoMostrar()" ' + visualizarListagemPLP ;
        <?php if ($numeroProtocoloFormatado == $formatoDTO->getStrProtocoloFormatado()) { ?>
        formato += ' disabled=disable';
        <?php } ?>
        formato += '>';
        formato += '            <label class="infraRadioLabel" for="rdoFormato_' + id + '"></label>';
        formato += '        </div>';
        ...
        //HTML para a coluna justificativa
        var justificativa = '<textarea name="txtJustificativa[' + id + ']" ' + checkImpressao + ' class="infraText" style="width: 200px;" value="<?php echo $formatoDTO->getStrJustificativa(); ?>" <?php echo $disabled; ?> ' + visualizarListagemPLP + '><?php echo $formatoDTO->getStrJustificativa(); ?></textarea>';
        ...
        objAutoCompletarProtocoloAnexo.processarResultado = function (id, descricao, complemento) {
            ...
            if (id != '') {
                ...
                if (i == options.length) {
                    ...
                    formato += '<input id="rdoImpresso_' + id + '" value="<?php echo MdCorExpedicaoFormatoRN::$TP_FORMATO_IMPRESSO; ?>" type="radio" name="rdoFormato_' + id + '" onclick="impressaoMostrar()">Impresso';
                    formato += '</label>';

                    formato += '<label for="rdoMidia_' + id + '" class="infraLabelRadio">';
                    formato += '<input id="rdoMidia_' + id + '" value="<?php echo MdCorExpedicaoFormatoRN::$TP_FORMATO_MIDIA; ?>" type="radio" name="rdoFormato_' + id + '" onclick="impressaoMostrar()">Gravao em Mdia';
                    formato += '</label>';
                    ...
                    impressao += '<input id="rdoImpressao1_' + id + '" value="<?php echo MdCorExpedicaoFormatoRN::$TP_IMPRESSAO_PRETO_BRANCO; ?>" type="radio" name="rdoImpressao_' + id + '">Preto e Branco';
                    impressao += '</label>';

                    impressao += '<label for="rdoImpressao2_' + id + '" class="infraLabelRadio">';
                    impressao += '<input id="rdoImpressao2_' + id + '" value="<?php echo MdCorExpedicaoFormatoRN::$TP_IMPRESSAO_COLORIDO; ?>" type="radio" name="rdoImpressao_' + id + '">Colorido';
                    impressao += '</label>';

                    //HTML para a coluna justificativa
                    var justificativa = '<textarea name="txtJustificativa[' + id + ']" class="infraText" style="width: 200px;"></textarea>';
    ...
    function impressaoMostrar() {
        ...
        for (var i = 0; i < arrSelect.length; i++) {
            ...
            var camposFormato = document.getElementsByName('rdoFormato_' + valorIdProtocoloAnexo);
            var camposImpressao = document.getElementsByName('rdoImpressao_' + valorIdProtocoloAnexo);
            var labelsImpressao = document.getElementsByName('lblImpressao_' + valorIdProtocoloAnexo);
    ...
    function recarregarContato(){
        var paramsAjax = {
            idContato: document.getElementById('selContato').value
        };
        $.ajax({
    ...
    function alterarContato() {
        seiAlterarContato(<?= $id_destinatario ?>, 'selContato', 'frmSolicitarExpedicao', '<?= $strLinkAlterarContato ?>');
        setTimeout(function(){
            recarregarContato()
        }, 3000)
    }
