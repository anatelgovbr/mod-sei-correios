...
try {
    ...
    $strTelefoneContato = '';
    $strEmailContato = $objMdCorExpedicaoSolicitadaDTO->getStrEmailSolicitante();
    ...
    if ($objMdCorExpedicaoSolicitadaDTO->getNumNumeroCnpj() != '') {
        ...
        if (!is_null($objContatoDTO)) {
            ...
            //se não existe do contato solicitante então pega o telefone do orgao
            //$strTelefoneContato = $strTelefoneContato == '' ? $objContatoDTO->getStrTelefoneFixo() : $strTelefoneContato;
            $strTelefoneContato = $strTelefoneContato == '' ? $objContatoDTO->getStrTelefoneCelular() : $strTelefoneContato;

            //se não existe do contato solicitante então pega o email do orgao
            $strEmailContato = $strEmailContato == '' ? $objContatoDTO->getStrEmail() : $strEmailContato;
        } else {
            $strCliente = '';
            $strTelefoneContato = '';
            $strEmailContato = '';
...
?>
<div>
    <img class="logo-correio" src="modulos/correios/imagens/logo-correio.png"  />
...
<table width="100%" style="border: 1px solid black; padding: 5px;border-collapse: collapse;">
    ...
    <tr>
        <td style="border-bottom: 1px solid black;padding: 5px;width: 80%;">
            ...
            <p><b>E-mail de Contato:</b> <?= $strEmailContato ?></p>
        </td>
