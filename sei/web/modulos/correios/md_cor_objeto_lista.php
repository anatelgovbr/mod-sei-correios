...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td>'.PaginaSEI::tratarHTML($arrObjMdCorObjetoDTO[$i]->getStrNomeTipoObjeto()).'</td>';
      $strResultado .= '<td>'.PaginaSEI::tratarHTML($arrObjMdCorObjetoDTO[$i]->getStrRotuloImpressao()).'</td>';
      $strResultado .= '<td>'.PaginaSEI::tratarHTML($arrObjMdCorObjetoDTO[$i]->getStrSinObjetoPadrao() =='S' ? 'Sim': 'Não').'</td>';
