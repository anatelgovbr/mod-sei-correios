...
$strResultado .= '<th class="infraTh" style="width: 200px  !important;">' . PaginaSEI::getInstance()->getThOrdenacao($mdCorRetornoArDTO, 'Data e Hora do Processamento', 'DataCadastro', $arrMdCorRetornoArDTO) . '</th>';
$strResultado .= '<th class="infraTh" style="width: 300px  !important;">' . PaginaSEI::getInstance()->getThOrdenacao($mdCorRetornoArDTO, 'Usuário', 'NomeUsuario', $arrMdCorRetornoArDTO) . '</th>';
$strResultado .= '<th class="infraTh" style="width: 80px !important;">Ações</th>';
$strResultado .= '</tr>';
...
foreach ($arrMdCorRetornoArDTO as $chave => $resultado) {
    ...
    $strResultado .= '<td valign="top">' . PaginaSEI::getInstance()->getTrCheck($chave, $resultado->getNumIdMdCorRetornoAr(), $resultado->getNumIdMdCorRetornoAr()) . '</td>';
    $strResultado .= '<td>';
    ...
    $strResultado .= '</td>';
    $strResultado .= '<td>';
    $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_cor_retorno_ar_resumir&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_md_cor_retorno_ar=' . $resultado->getNumIdMdCorRetornoAr()) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="modulos/correios/imagens/png/associar.png" title="Resumo do Processamento" alt="Resumo do Processamento" class="infraImgAcoes" /></a>&nbsp;';
//    if ($resultado->getStrSinAutenticado() == 'S') {
//         $strResultado .= '<a  tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="modulos/correios/imagens/icon_autenticado.png" title="ARs Autenticados" alt="ARs Autenticados" class="infraImg" /></a>&nbsp;';
//    } else {
