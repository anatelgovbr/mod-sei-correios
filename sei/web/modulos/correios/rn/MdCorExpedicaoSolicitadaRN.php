...
class MdCorExpedicaoSolicitadaRN extends InfraRN
{
    ...
    protected function cadastrarControlado(MdCorExpedicaoSolicitadaDTO $objMdCorExpedicaoSolicitadaDTO)
    {
        ...
        try {
            ...
            // Os filtros do módulo comparam estes sinalizadores com 'N' e descartariam linhas
            // com NULL, o que faria a solicitação sumir das regras de AR pendente.
...
//        $objMdCorExpedicaoSolicitadaDTO = new MdCorExpedicaoSolicitadaDTO();
    ...
    /**
     * Identifica, entre os processos informados, quais possuem solicitação de expedição com
     * AR ainda pendente de retorno, em qualquer etapa: aguardando expedição, em procedimento
     * de postagem ou já postado aguardando o retorno do AR.
     *
     * @param array $arrObjProcedimentoAPI lista de ProcedimentoAPI avaliada na operação
    ...
    public function validarExistenciaObjetoAguardandoRetornoAR($arrObjProcedimentoAPI) {
        ...
        // A infrigência é lida por código de rastreamento, e não pelo relacionamento do DTO:
        // aquele relacionamento amarra apenas pelo documento e aproveitaria o retorno de AR
        // de outra expedição do mesmo documento.
        ...
        if (!empty($arrMdCorExpedicaoSolicitadaDTO)) {
            foreach ($arrMdCorExpedicaoSolicitadaDTO as $mdCorExpedicaoSolicitadaDTO) {
                ...
                // Processo já bloqueado por outro objeto: não precisa reavaliar
                if (isset($arrProtocoloBloqueado[$idProcedimento])) {
                    continue;
                }

                // A pendência começa na solicitação: objeto aguardando expedição ou em
                // procedimento de postagem também bloqueia, porque o AR ainda vai retornar.

                // Se o retorno de AR DESTA expedição registrou infrigência -> não bloqueia
                ...
                // Rastreamento do objeto, obtido em consulta única antes do laço.
                // 'I' = Insucesso/Extraviado: o AR não vai retornar -> não bloqueia
                // 'S' = Entregue NÃO libera: a entrega do objeto não garante o retorno do AR digitalizado
                ...
                // Se passar por todos os "continues" acima, a solicitação tem AR pendente de
                // retorno: aguardando expedição, em postagem ou já postada aguardando o AR.
    ...
    /**
     * Obtém a situação de rastreio do último andamento de cada solicitação em consulta única,
     * evitando uma consulta por solicitação dentro do laço de validação.
     *
     * A situação vem de md_cor_lista_status e é resolvida uma vez por par (status, tipo)
     * distinto. Usa consultar, e não listar, porque o recurso de listagem dessa tabela é
     * exclusivo do perfil Administrador.
     *
     * @param array $arrIdMdCorExpedicaoSolicitada solicitações avaliadas
     * @return array mapa id da solicitação => situação de rastreio do módulo
     */
    private function _obterStaRastreioPorSolicitacao($arrIdMdCorExpedicaoSolicitada)
    {
        ...
        // ordenação decrescente: o primeiro andamento de cada solicitação é o mais recente
        $arrUltimoAndamento = [];
    ...
    /**
     * Indexa a infrigência registrada no retorno de AR pelo código de rastreamento do objeto,
     * garantindo que o retorno de uma expedição não seja atribuído a outra expedição do
     * mesmo documento.
     *
     * @param array $arrCodigoRastreamento códigos de rastreamento das solicitações avaliadas
     * @return array mapa código de rastreamento => id da infrigência (ou null)
     */
    private function _obterInfrigenciaRetornoArPorRastreamento($arrCodigoRastreamento)
    {
        ...
        foreach ($arrObjMdCorRetornoArDocDTO as $objRetornoArDocDTO) {
            // ordenação ascendente: o retorno mais recente sobrescreve os anteriores
