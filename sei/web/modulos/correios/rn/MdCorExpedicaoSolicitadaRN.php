...
class MdCorExpedicaoSolicitadaRN extends InfraRN
{
    ...
    private function validarStrObservacao(MdCorExpedicaoSolicitadaDTO $objMdCorExpedicaoSolicitadaDTO, InfraException $objInfraException)
    {
        if (InfraString::isBolVazia($objMdCorExpedicaoSolicitadaDTO->getStrObservacao())) {
            $objMdCorExpedicaoSolicitadaDTO->setStrObservacao(null);
        } else {
            $objMdCorExpedicaoSolicitadaDTO->setStrObservacao(trim($objMdCorExpedicaoSolicitadaDTO->getStrObservacao()));

            if (strlen($objMdCorExpedicaoSolicitadaDTO->getStrObservacao()) > 250) {
    ...
    protected function cadastrarControlado(MdCorExpedicaoSolicitadaDTO $objMdCorExpedicaoSolicitadaDTO)
    {
        ...
        try {
            ...
            $this->validarStrObservacao($objMdCorExpedicaoSolicitadaDTO, $objInfraException);
...
//            }
    ...
    protected function alterarControlado(MdCorExpedicaoSolicitadaDTO $objMdCorExpedicaoSolicitadaDTO)
    {
        try {
            ...
            if ($objMdCorExpedicaoSolicitadaDTO->isSetStrObservacao()) {
                $this->validarStrObservacao($objMdCorExpedicaoSolicitadaDTO, $objInfraException);
            }
