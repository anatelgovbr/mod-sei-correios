...
try {
    ...
    switch ($_GET['acao']) {
        case 'md_cor_plp_detalhada':
            ...
            $arrComandos[] = '<button type="button" accesskey="t" id="btnArquivoLote" onclick="imprimirArquivolLote(); setTimeout(function(){window.location.reload()}, 500);" class="infraButton">
    ...
    if($processar) {
        ...
        if ($numRegistros > 0) {
            ...
            $strResultado .= '<th class="infraTh" width="9%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Unidade Solicitante', 'SiglaUnidade', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";

            $strResultado .= '<th class="infraTh" width="9%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Data da Solicitação', 'DataSolicitacao', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";

            if ($_GET['acao'] == 'md_cor_plp_detalhada') {
                $strResultado .= '<th class="infraTh" width="5%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Data da Expedição', 'DataExpedicao', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";
            }

            $strResultado .= '<th class="infraTh" width="10%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Documento Principal', 'ProtocoloFormatadoDocumento', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";
            $strResultado .= '<th class="infraTh" width="10%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Processo', 'ProtocoloFormatado', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";
            $strResultado .= '<th class="infraTh" width="5%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Rastreamento', 'CodigoRastreamento', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";
            $strResultado .= '<th class="infraTh" width="5%">' . PaginaSEI::getInstance()->getThOrdenacao($objMdCorPlpDTO, 'Anexos', 'CodigoRastreamento', $arrObjMdCorExpedicaoSolicitadaDTO) . '</th>' . "\n";
            ...
            for ($i = 0; $i < $numRegistros; $i++) {
                ...
                if ($arrObjMdCorExpedicaoSolicitadaDTO[$i]->getStrMidia() == 'S') {
                    $midia = '<div style="float: left"><img src="modulos/correios/imagens/png/midia.png" title="PLP possui midia para gravação." style="width: 24px; height: 24px" alt="PLP possui midia para gravação." class="infraImg" /></div>';
                }
